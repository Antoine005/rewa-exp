...
def analyze_excel_data(name):
    ...
    import glob
    folder_path = "data"
    skip_csv_files = glob.glob(folder_path + f"/{name}_2*.csv")
    no_skip_csv_files = glob.glob(folder_path + f"/{name}_1*.csv")
    ...
    df_no_skip.loc[df_no_skip['VideoshowRT'].isna(), 'corr'] = -1
    ...
    try:
        ...
        try:    
            df_no_skip['corr'] = df_no_skip[['response_2.corr', 'response_3.corr']].sum(axis=1)
        except:
            try:
                df_no_skip['corr'] = df_no_skip[['response_3.corr', 'response_4.corr']].sum(axis=1)
            except:
                print('')
    try:
    ...
    df_skip.loc[df_skip['VideoshowRT'].isna(), 'corr'] = -1
    ...
    try:
        ...
        try:    
            df_skip['corr'] = df_skip[['response_2.corr', 'response_3.corr']].sum(axis=1)
        except:
            try:
                df_skip['corr'] = df_skip[['response_3.corr', 'response_4.corr']].sum(axis=1)
            except:
                print('')
    try:
